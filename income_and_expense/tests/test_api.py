...
class InternalHelperTests(TestCase):
    ...
    def test_can_update_or_delete_boundaries(self):
        y, m = shift_month(*current_year_month(), delta=-1)
        self.assertTrue(api_views._can_update_or_delete(y, m))
        y, m = shift_month(*current_year_month(), delta=-2)
        self.assertFalse(api_views._can_update_or_delete(y, m))
...
class IncomeViewSetTests(APITestCaseBase):
    ...
    def test_update_rejected_for_old(self):
        old = _today() - relativedelta(months=3)
        inc = make_income(name='x', method=self.method, amount=1, pay_date=old)
        res = self.client.patch(f'/api/incomes/{inc.pk}/', data={'amount': 2}, format='json')
        self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_full_update_old_rejected(self):
        old = _today() - relativedelta(months=3)
        inc = make_income(name='x', method=self.method, amount=1, pay_date=old)
        res = self.client.put(f'/api/incomes/{inc.pk}/', data={
            'name': 'y',
...
class ExpenseViewSetTests(APITestCaseBase):
    ...
    def test_update_rejects_old(self):
        old = _today() - relativedelta(months=3)
        exp = make_expense(
